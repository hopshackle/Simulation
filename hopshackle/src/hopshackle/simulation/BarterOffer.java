...
public class BarterOffer extends Offer {
	...
	public void resolve() {
		...
		if (bestBid != null && !bestBid.getBuyer().isDead()) {
			...
			String message = String.format("Buys %s for %s (%.2f)", item.toString(), valFunction.toString(bestBid.getBarterItems()), bestBid.getAmount());
			if (numberOfItem > 1) 
				message = String.format("Buys %d %s for %s (%.2f)", (int)numberOfItem, item.toString(), valFunction.toString(bestBid.getBarterItems()), bestBid.getAmount());
			buyer.log(message);

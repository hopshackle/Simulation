...
public class WorldMap extends SimpleAnimationPanel {
	...
	public WorldMap(World w) {
		...
		if (w.getLocationMap() == null || !(w.getLocationMap() instanceof SquareMap))
			throw new AssertionError ("Must have a World with a square map");
	...
	public void drawFrame(Graphics g) {
		...
		List<Location> allS = w.getChildLocations(); 
		for (Location l : allS) {
	...
	private Square getSquare(int x, int y) {
		int s_x = x / cellSize;
		int s_y = y / cellSize;
		SquareMap map = (SquareMap) w.getLocationMap();
		return map.getSquareAt(s_x, s_y);
	}
